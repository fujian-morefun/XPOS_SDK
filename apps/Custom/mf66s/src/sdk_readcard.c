...
void sdk_SetTermConfig()
{
	...
	memcpy( termconfig.TermCap, "\x00\x08\xC8", 3);	/*Terminal performance '9F33'*/
	...
	memcpy(termconfig.szTransProp,"\x30\x00\xC0\x00",4);				/*TTQ'9F66'*/
...
void clear_service_prmacqkey(void)
{
	...
	{
		gui_messagebox_show("", "clear succ!" , "" , "ok" , 0);
	}
	else
	{
		gui_messagebox_show("", "clear fail!" , "" , "ok" , 0);
	}
}
...
void init_service_prmacqkey(int binitial)
{
	...
	if(nret != SUCC)
	{
		gui_messagebox_show("", "download fail!" , "" , "ok" , 0);
		return;
	}
	...
	if(nret == SUCC)
	{
		gui_messagebox_show("", "download succ!" , "" , "ok" , 0);
	}
	else
	{
		gui_messagebox_show("", "download fail!" , "" , "ok" , 0);
...
int upay_readcard_proc(st_read_card_in *card_in, st_read_card_out *card_out)
{
	...
	//rf_icon_lcd0_paint();			//lcd0 display rf icon 
...
loop_card:
	...
	if(EMVAPI_RET_SEEPHONE==ret)//visa require see phone and retry
	{
	...
	main_page_lcd0_paint();		//main_page_lcd0_paint
	...
	{
		Play_Voice("paysuc");	//Payment successful
