...
static void _inputamount_PaintAmount(char* amount)
{
	...
	//XGUI_SET_WIN_RC;
	gui_clear_rect(0, GUI_LINE_TOP(1) , gui_get_width() , GUI_LINE_TOP(2), gui_get_bg_color());
	gui_text_out(gui_get_width() - textWidth - 10, GUI_LINE_TOP(1), amount);
...
static void _inputamount_PanitAll(char* title, int nAmount)
{
	...
	input_PaintTitle("Input the Amount:");
	_setAmount(nAmount);
	gui_end_batch_paint();
}
