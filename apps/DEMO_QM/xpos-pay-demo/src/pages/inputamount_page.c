	...
	case KEY_9:
		{
			...
			else
			{
				if (LCD_IS_320_240)
				{
					sprintf(buf, "Amount not>%ddigit", maxLength);
				} 
				else
				{
					sprintf(buf, "Amt not>%ddigit", maxLength);
				}
	...
	default:
		{
			if (LCD_IS_320_240)
			{
				input_PaintFooter0("Enter right amount");
			} 
			else
			{
				input_PaintFooter0("Enter right amt");
			}
		}
		break;
	}
			...
			if (pMsg.MessageId == XM_GUIPAINT) {
				...
				else if(presskey == KEY_OK)
				{
					if (nAmount == 0)
					{
						...
						{
							input_PaintFooter0("Amt cannot be 0");
						}
					}
					else
					{
