...
// 	CAPUBLICKEY pkKey={0};
	...
	//if ( amt <= 0 ) return -1;
	amt = inputamount_page(title, 9, 90000);
	if(amt <= 0)
	{
		return -1;
	}
	...
	else if (ret == 2)//Find a non-contact card
	{
		...
		strcpy(card_in->card_page_msg, "Please swipe card");//Swipe interface prompt information, a line of 20 characters, up to two lines, automatic branch.
