...
extern "C"{
	...
	/**< -----------------------------------------------------------------------------------------------------*/
	/**<终端参数ST_TERMPARA结构cTestType测试项目参数*/
...
#define KERNEL_RUPAY		0x80		
	/**< 终端参数ST_TERMPARA结构cPackType打包类型参数*/
...
#define EMV_ONLINE_FAIL			(-1)		/**< 联机失败*/
	...
	typedef struct {
		...
		//DF69 标识是否支持国密算法 国密算法是不是不通过AID来配置？
		char cResv[15];	
#else
		char szCurCode_aid_5F2A[2];			/**<(TERM)交易货币代码*/
		char c5F36_len;
...
#define OFFLINE_PIN_NOMAL	0x03
#define OFFLINE_PIN_AGAIN	0x02
#define OFFLINE_PIN_LAST	0x01	
#define	ONLINE_PIN			0x00
	...
	LIB_EXPORT extern int VcpsProc_set(ST_VCPS_PROC*qparam);
	LIB_EXPORT extern int Qpboc_set(ST_QPBOC_PROC*qparam);
	
	/**<log文件*/
	LIB_EXPORT extern int Init_TransLog(void);

	LIB_EXPORT int Vcps_Sel_App_issuser();
	LIB_EXPORT int Vcps_Init_Issuser_Res(STEMVPROC *stProc);
	...
	//转存授权金额
	LIB_EXPORT extern int SetAuthAmt(char *psAmt);
	/**< 获取EMV错误码*/
	...
	/**< RF */
	LIB_EXPORT extern int EMV_Rf_PreProcess(STEMVPROC *stProc,char *pszAid,char nAidLen);
	LIB_EXPORT extern int EMV_Rf_Trans(STEMVPROC *stProc);
	LIB_EXPORT extern int EMV_Rffail_Trans(STEMVPROC *stProc,int bAll);//闪卡重新处理流程
	LIB_EXPORT extern int EMV_Rf_Final(STEMVPROC *stProc);
	LIB_EXPORT extern int EMV_Rf_End(void);

	LIB_EXPORT int Qpass_add_begin(int binType, int max);
	LIB_EXPORT int Qpass_add_end(int binType);
	...
	LIB_EXPORT int Ex_TLV_GetDataByTag(BYTE *psTag, BYTE *psSrc, int nSrcLen,BYTE *psBuf, int *nBufLen);

	LIB_EXPORT int UpCard_Deal(char* pszPan, char* pszTrack2, char* pszTrack3, char* pszExpDate, char* pszField55, int* pnField55Len);
...
# define EMV_INS_ISS_RECOVER_CHECK			 0xB2 // 发卡行回收证书检查
...
#define EMV_MSGID_CLEAR_DISPLAY				 0x1E // 清空显示 /  Clear screen display
	...
	//管理回收CA公钥证书0x01-添加；0x02-删除; 0x03 - 全部删除
	LIB_EXPORT extern int EMV_iManageRecCert(char ucAction, EMV_stRecCert *ptRecCert);
	...
	//LIB_EXPORT extern int EMV_SetEmvInterfaceFunP(ST_EMV_EXPAND_INTERFACE *stEMVInterface);

	//设置终端TLV标签数据
	LIB_EXPORT extern int EMV_iSetTLV(char *pszTag,char *pszData,int iLen);
	LIB_EXPORT extern int EMV_iSetTLVList(char *pszTagList,int iLen);
	LIB_EXPORT extern int EMV_RF_TransEx(STEMVPROC *stProc);	//非接交易
	LIB_EXPORT extern int EMV_iSetCAPubKey(ST_CAPK*stCAPK);
	LIB_EXPORT extern int EMV_iForceKernelId(int iKernelId);	//认证使用
	LIB_EXPORT extern int EMV_iSetTLVList_ACT(char *pszTagList,int iLen);	//交易开始之前设置相关交易参数
	LIB_EXPORT extern int PayPass_PreProcess(STEMVPROC *stProc);
	LIB_EXPORT extern int EMV_iCreate(void);					//EMV初始化资源
	LIB_EXPORT extern int EMV_iDestroy(void);					//EMV资源销毁


	//量产外卡API接口
	LIB_EXPORT extern int EMV_Rf_PreProcessEx2(STEMVPROC *stProc,EMV_stTermAIDList *pstTermAid,int iAidCount);
	LIB_EXPORT extern int EMV_Rf_TransEx2(STEMVPROC *stProc);
	LIB_EXPORT extern int EMV_Rf_FinalEx2(STEMVPROC *stProc);
	LIB_EXPORT extern int EMV_Rf_EndEx2(void);
