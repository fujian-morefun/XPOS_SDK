#ifndef _RFID_ASIC_RC531_H
#define _RFID_ASIC_RC531_H
...
struct rfid_asic_rc531_transport {
	struct {
...
struct rc531_transport_handle {
  unsigned int dump;
};

/* A handle to a specific RC531 chip */
struct rfid_asic_rc531_handle {
	struct rc531_transport_handle th;
	int fd_rc531;
};

struct rfid_asic_rc531_impl_proto {
...
struct rfid_asic_rc531_impl {
	u_int32_t mru;		/* maximum receive unit (PICC->PCD) */
	u_int32_t mtu;		/* maximum transmit unit (PCD->PICC) */
	//struct rfid_asic_rc531_impl_proto proto[NUM_RFID_PROTOCOLS];
};

extern struct rfid_asic_handle * rc531_open(struct rfid_asic_transport_handle *th);
extern void rc531_close(struct rfid_asic_handle *h);
