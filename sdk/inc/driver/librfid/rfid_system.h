...
#define free_asic_handle(x)	do {memset(&rfid_ah, 0x0, sizeof(rfid_ah));} while(0)
...
#define free_layer2_handle(x)	do {memset(&rfid_l2h, 0x0, sizeof(rfid_l2h));} while(0)
...
#define free_protocol_handle(x)	do {memset(&rfid_ph, 0x0, sizeof(rfid_ph));} while(0)
...
#define free_rat_handle(x)	do {memset(&rfid_ath, 0x0, sizeof(rfid_ath));} while(0)


extern struct rfid_reader_handle rfid_rh;
#define malloc_reader_handle(x)	&rfid_rh
//#define free_reader_handle(x)	EMPTY_STATEMENT
#define free_reader_handle(x)	do {memset(&rfid_rh, 0x0, sizeof(rfid_rh));} while(0)
